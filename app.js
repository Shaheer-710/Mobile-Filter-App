let mobiles = {
  ...
  function selectBrand() {
    ...
    for(let i=0; i<allModels.length;i++){
      models.innerHTML += `<option value="${allModels[i]}">${allModels[i]}</option>`
    }
  ...
  function getdata() {
    ...
    // Access the selected model's data
    let modelData = mobiles[selectedBrand][selectedModel];

    // Create a container to display the details
    let detailsContainer = document.getElementById('details');
    detailsContainer.innerHTML = ''; // Clear previous details

    // Iterate through the model's properties and display them
    for (let property in modelData) {
        if (typeof modelData[property] === 'object') {
            // Handle nested objects (e.g., memory, camera)
            for (let subProperty in modelData[property]) {
                detailsContainer.innerHTML += `<p>${subProperty}: ${modelData[property][subProperty]}</p>`;
            }
        } else {
            detailsContainer.innerHTML += `<p>${property}: ${modelData[property]}</p>`;
        }
    }
}
